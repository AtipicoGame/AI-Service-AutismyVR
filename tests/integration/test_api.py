...
@pytest.fixture
def mock_ollama():
    with patch.object(ChatService, '__init__', lambda x: None):
        service = ChatService()
        service.ollama_client = MagicMock()
        service.title_service = MagicMock()
        service.title_service.generate_title = MagicMock(return_value="Test Title")
        service.ollama_client.request = MagicMock(return_value={"content": "Test Response", "total_duration": 1.0})
        with patch('src.services.chat_service.ChatService', return_value=service):
            yield service
